...
describe('name', function () {
  ...
  describe.only('work with dht using a single node', () => {
    let node
    let nodeId
    let ipfsd
  ...
  describe('work with dht using multiple nodes', () => {
    let nodes
    let nodeA
    let nodeB
    let nodeC
    ...
    it('should recursively resolve to an IPFS hash', function (done) {
      ...
      nodeA.key.gen(keyName, { type: 'rsa', size: 2048 }, function (err, key) {
        ...
        nodeA.name.publish(ipfsRef, { resolve: false }, (err) => {
          expect(err).to.not.exist()

          nodeA.name.publish(`/ipns/${idA.id}`, { resolve: false, key: keyName }, (err) => {
            expect(err).to.not.exist()

            nodeB.name.resolve(key.id, { recursive: true }, (err, res) => {
              expect(err).to.not.exist()
              expect(res).to.exist()
              expect(res.path).to.equal(ipfsRef)
              done()
